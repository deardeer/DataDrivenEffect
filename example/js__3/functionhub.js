function FunctionHub(example){
	...
	this._init = function(example){
		console.log(' init funcHub ', example)
		this._currentExample = example
		this._geometryDefiner[this._currentExample] = new GeometryDefiner();
		this._geometrySaver[this._currentExample] = new GeometrySaver(this);
		this._animateEditor[this._currentExample] = new AnimateEditor(this);
		this._animateEnhancer[this._currentExample] = new AnimateEnhancer();
		this._animateDomManager[this._currentExample] = new AnimateDomManager(this);

		//load from server
		var maConfigName = example // "boxplot2";
	...
	this.successFetchFunc = function(response, self){
		...
		var drawPath = true;//response['ma']['drawpath'];
			...
			for(var j = 0; j < malist.length; j ++){
				...
				for(var p = 0; p < liGeometryType.length; p ++){
					...
					if(ma[type]!=undefined){
						this._geometrySaver[this._currentExample].saveGeometryNew({'type': type, 'geotype': ma[type]['geoType']}, ma[type]['dots']);
				...
				//fit ant modal
				var currentPathGeometry = this._geometrySaver[this._currentExample].getCurrentMAGeometry()['path'];
				this._geometrySaver[this._currentExample].fitAnt(antModal, currentPathGeometry, 'area', 0, false);
				// this.setFunc({type: 'addGeometry'});
				d3.selectAll('.define_geometry').style('visibility', 'hidden');
				
				//ma
				var MAGeometryInfo = this._geometrySaver[this._currentExample].getCurrentMAGeometry();
				// console.log(" MAGeometryInfo ", MAGeometryInfo, aeInfo);
				var MAGeometryIndex = this._geometrySaver[this._currentExample].addMAGeometry(MAGeometryInfo);
				this._liStoreMAIndex.push(MAGeometryIndex);
				var animateResult = this._animateEnhancer[this._currentExample].activateDOM(MAGeometryInfo, aeInfo);
				var metainfo = animateResult['metainfo'];

				//clear current MAGeometry
				//not exist before
				this._geometrySaver[this._currentExample].clearCurrentMAGeometry();
				this._animateDomManager[this._currentExample].addMetaInfo(MAGeometryIndex, metainfo)
				this._animateEditor[this._currentExample].addMADoms(MAGeometryIndex, metainfo);
			}
		}
	}
	this.addMAbyGroupInfo = function(groupMA, drawPath){
	...
	this.initFunc = function(){
		...
		switch(this._type){
			...
			case 'new':
			case 'editGeometry':
				this._geometryDefiner[this._currentExample].beginMode(this._para);
				break;

			case 'addGeometry': 
			    console.log(" add geo! ", this._para, this._geometryDefiner.getGeometry());
				this._geometrySaver[this._currentExample].saveGeometryNew(this._para, this._geometryDefiner.getGeometry()); 
				this._geometryDefiner[this._currentExample].clearMode()
				break;
			...
			case 'editanimate':
				...
				var aeInfo = this._animateEditor[this._currentExample].editAnimate();

				// if(this._currentMAGeometryId != null){
				// 	console.log(" [2] edit animte ", this._currentMAGeometryId);
					
				// 	// var liAnimateDom = this._animateEnhancer[this._currentExample].activateDOM(this._geometrySaver.getGeometry(this._currentGeometryId), aeInfo);
				// }
				break;
			...
			case 'animate':
				...
				_this._currentIntervalId = setInterval(function(){
					...
					// var liAnimateDoms = _this._animateDomManager[this._currentExample].getAnimateDoms();					
					var mapMetaInfo = _this._animateDomManager[_this._currentExample].getMetaInfos();
					...
					for(var i = 0; i < liMAIndex.length; i ++){
						...
						var metaInfo = _this._animateDomManager[_this._currentExample].getMetaInfobyIndex(maIndex);
						...
						for(var j = 0; j < liAntOffset.length; j ++){ //
							...
							var currentAnt = _this._animateDomManager[_this._currentExample].getAnt(maIndex, offset);
							...
							if(j == 0 && offset >= (antGap - antWidth/2.)){
								...
								var ant = _this._animateDomManager[_this._currentExample].getAnt(maIndex, newOffset);
								ant.visible = true;
								liNextAntOffset.push(newOffset);
							}

							var newAnt = _this._animateDomManager[_this._currentExample].getAnt(maIndex, offset);
							newAnt.visible = true;
						...
						_this._animateDomManager[_this._currentExample].updateMetaInfo(maIndex, metaInfo);
			...
			case 'upload': 
				var printStr = "[";
				var liGeometry = this._geometryDefiner[this._currentExample].getGeometry();
			...
			case 'saveDB':
				...
				var result = {};
				result['name'] = name;
				var mapIndexMAInfo = _this._animateDomManager[this._currentExample].getMetaInfos();
				...
				for(var index in mapIndexMAInfo){
					...
					var geometry = _this._geometrySaver[this._currentExample].getMAGeometrybyIndex(index);
